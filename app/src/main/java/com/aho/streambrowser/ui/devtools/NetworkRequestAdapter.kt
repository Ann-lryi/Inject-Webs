...
class NetworkRequestAdapter(
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): VH {
        ...
        fun TextView.lp(w: Int, g: Int = Gravity.START) {
            layoutParams = LinearLayout.LayoutParams(
                if (w == 0) 0 else dp(w), LinearLayout.LayoutParams.WRAP_CONTENT,
                if (w == 0) 1f else 0f).apply { gravity = g }
        }
        val icon = TextView(ctx).apply { textSize = 11f }.also { it.lp(16, Gravity.CENTER) }
        val method = TextView(ctx).apply {
            textSize = 9.5f; setTextColor(0xFF888888.toInt()); maxLines = 1
        }.also { it.lp(40) }
        val hostCol = LinearLayout(ctx).apply {
            orientation = LinearLayout.VERTICAL; setPadding(dp(4), 0, dp(4), 0)
            layoutParams = LinearLayout.LayoutParams(0, LinearLayout.LayoutParams.WRAP_CONTENT, 1f)
        }
        ...
        val status = TextView(ctx).apply { textSize = 9.5f; maxLines = 1 }
            .also { it.lp(32, Gravity.CENTER) }
        val type = TextView(ctx).apply {
            textSize = 8.5f; setTextColor(Color.BLACK); maxLines = 1
            setPadding(dp(3), dp(1), dp(3), dp(1))
        }.also { it.lp(38, Gravity.CENTER) }
        val size = TextView(ctx).apply {
            textSize = 9f; setTextColor(0xFF888888.toInt()); maxLines = 1
        }.also { it.lp(42, Gravity.END) }
        val waterfall = FrameLayout(ctx).apply {
    ...
    class VH(
        ...
        fun bind(req: NetworkRequest, t0: Long, tRange: Long) {
            ...
            val totalPx = (60 * density).toInt()
            val rel = ((req.timestamp - t0).toFloat() / tRange).coerceIn(0f, 0.9f)
            waterfall.addView(View(itemView.context).apply {
                layoutParams = FrameLayout.LayoutParams((totalPx * rel).toInt(), FrameLayout.LayoutParams.MATCH_PARENT)
            })
            waterfall.addView(View(itemView.context).apply {
                setBackgroundColor(typeCol)
                layoutParams = FrameLayout.LayoutParams(
                    (totalPx * (0.08f + rel * 0.02f).coerceAtMost(0.15f)).toInt(),
